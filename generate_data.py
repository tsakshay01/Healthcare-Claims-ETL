...
# Possible Diagnosis Codes (ICD-10 examples)
...
for i in range(NUM_ROWS):
    ...
    # Patient Name: Sometimes uppercase, sometimes lowercase (Dirty Data)
    patient_name = fake.name()
    if random.choice([True, False]):
        patient_name = patient_name.lower()
    
    # Provider NPI: Should be 10 digits. We will intentionally make some '9 digits' to simulate errors.
    if random.random() < 0.95: # 95% chance of correct data
        provider_npi = str(random.randint(1000000000, 9999999999))
    else:
        provider_npi = str(random.randint(100000000, 999999999)) # Invalid 9 digit NPI
        
    diagnosis_code = random.choice(diagnosis_codes)
    
    # Claim Amount: Random float between $50 and $12,000
    claim_amount = round(random.uniform(50.0, 12000.0), 2)
    
    status = random.choice(statuses)
    
    # Introduce Null Values (Dirty Data)
    # 2% chance the Status is missing
...
# Create Explicit Duplicates
# We repeat the first 50 rows to simulate data entry errors
